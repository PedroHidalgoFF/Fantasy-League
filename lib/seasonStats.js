...
// playerId -> puntos reales de UNA semana específica (no acumulados)
export async function getWeekStatsByPlayer(season, week) {
  const data = await getJSON(
    `${STATS_BASE}/${season}/${week}?season_type=regular&${positionQueryString()}`
  );
  ...
  for (const entry of data) {
    const pts = entry.stats?.pts_ppr;
    if (entry.player_id && typeof pts === "number") {
      stats[entry.player_id] = Math.round(pts * 10) / 10;
    }
  }
  return stats;
}
