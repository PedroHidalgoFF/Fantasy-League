...
export function buildRelevantPlayerIndex(players) {
  return Object.entries(players)
    .filter(([, p]) => typeof p.search_rank === "number" && p.search_rank < 600 && p.last_name && p.first_name)
    .map(([playerId, p]) => ({
      playerId,
      firstNameLower: p.first_name.toLowerCase(),
...
// Busca en el CONTENIDO completo de la noticia (título + resumen) al primer
// jugador conocido cuyo nombre Y apellido aparezcan juntos (ej. "Patrick
// Mahomes"), no solo el apellido suelto — así evitamos emparejar con la
// foto equivocada cuando el apellido es común o coincide con otra palabra.
export function findFeaturedPlayerForArticle(item, playerIndex) {
  const text = stripHtml(`${item.title} ${item.description}`).toLowerCase();
  for (const entry of playerIndex) {
    if (entry.lastNameLower.length < 4) continue;
    const fullNameRegex = new RegExp(`\\b${entry.firstNameLower}\\s+${entry.lastNameLower}\\b`);
    if (fullNameRegex.test(text)) return entry;
  }
  return null;
}
